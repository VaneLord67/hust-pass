...
func TestSpider(t *testing.T) {
	...
	c.OnHTML("#lt", func(e *colly.HTMLElement) {
		lt = e.Attr("value")
	})
	c.OnHTML("[name=\"execution\"]", func(e *colly.HTMLElement) {
		execution = e.Attr("value")
	})
	c.Visit("https://pass.hust.edu.cn/cas/login")
	collyCookies := c.Cookies("https://pass.hust.edu.cn")
	var ipPool string
	var jsid string
	for _, cookie := range collyCookies {
		switch cookie.Name {
		case "JSESSIONID":
			jsid = cookie.Value
	...
	rsa, err := GetRSA(config.GlobalConfig.Username, config.GlobalConfig.Password, lt)
	if err != nil {
		log.Fatal(err)
	}
...
func GetRSA(userName, password, lt string) (string, error) {
	vm := otto.New()
	_, err := vm.Run(Script(userName, password, lt))
	if err != nil {
		return "", err
	}
	...
	res, err := value.ToString()
	if err != nil {
		return "", err
	}
	return res, nil
}
func Script(userName, password, lt string) string {
	argument := fmt.Sprintf("\"%s\"+\"%s\"+\"%s\"", userName, password, lt)
	a := `res = strEnc(` + argument + `, '1', '2', '3')
	`
	b := `
	res = strEnc("U202015037"+"skcjr.0910."+"LT-6311-yoZexUttQmcM4ID4ciQUi0pfbfI0JC-cas", '1', '2', '3')
